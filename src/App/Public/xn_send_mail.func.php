...
class PHPMailer
{
	...
	public function EncodeQ($str, $position = 'text')
	{
		...
		switch (strtolower($position)) {
			case 'phrase':
				$encoded = preg_replace_callback("/([^A-Za-z0-9!*+\/ -])/", [$this, 'sprintf_1_callback'], $encoded);
				break;
			case 'comment':
				$encoded = preg_replace_callback("/([\(\)\"])/", [$this, 'sprintf_1_callback'], $encoded);
			case 'text':
			default:
				// Replace every high ascii, control =, ? and _ characters
				//TODO using /e (equivalent to eval()) is probably not a good idea
				$encoded = preg_replace_callback('/([\000-\011\013\014\016-\037\075\077\137\177-\377])/',
					[$this, 'sprintf_2_callback'], $encoded);
				break;
		}
...
*/
function xn_send_mail($smtp, $username, $email, $subject, $message, $file = '', $charset = 'UTF-8')
{
	// 部分 SMTP 不支持UTF-8
	/*
