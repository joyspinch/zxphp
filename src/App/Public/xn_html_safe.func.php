...
{
	private $_stack = [];    //
	private $_dcStack = [];    // 删除的栈
	private $_dcCounter = [];    // 删除的标签数
	private $_xhtml = '';
	private $_counter = [];         // 打开的标签数
	private $tableTags = ['caption', 'col', 'colgroup', 'tbody', 'td', 'tfoot', 'th', 'thead', 'tr'];
	...
	private $listTags = ['dir', 'menu', 'ol', 'ul', 'dl'];
	private $_liStack = [];
	private $_listScope = 0;

	private $singleTags = ['area', 'br', 'img', 'input', 'hr', 'wbr'];
	private $noClose = [];
	...
	private function _writeAttrs($attrs, $tagname)
	{
		...
		foreach ($attrs as $name => $value) {
			if ($name == 'style') {
				...
				$csskv = !empty($m[1]) && !empty($m[2]) ? array_combine($m[1], $m[2]) : [];
				...
				$value = $ok ? $value : $v[1];
				// 白名单
			} else if (isset($this->white_value[$name])) {
				$v = $this->white_value[$name];
				if ($v[0] == 'range') {
					$px = 0;
			...
			$tempval = preg_replace_callback('/&#(\d+);?/m', [$this, 'chr_callback'], $value); //"'
			$tempval = preg_replace_callback('/&#x([0-9a-f]+);?/mi', [$this, 'chr_hexdec_callback'], $tempval);
