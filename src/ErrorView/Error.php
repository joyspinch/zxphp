...
<head>
	<meta charset="utf-8">
	<title><?php echo $result?$result:($error['message']?$error['message']:'这是一个错误');?></title>
	<style>
		html, body, div, h1, * {
			margin: 0;
			padding: 0;
...
<div class="box">
	<?php if (_CONF('debug')) {  ?>
		<?php if (isset($error['file'])) { ?>
			<h1>Error File : <?php echo str_replace(__ROTDIR__, '', $error['file']), '(', $error['line'], ')' ?></h1>
			<p style="margin-top:20px"> <?php echo $error['message'] ?></p>
		<?php } else { ?>
			<p style="margin-top:20%;font-size:20px;text-align:center"><?php echo $result ?></p>
		<?php } ?>
	<?php } else { ?>
		<?php if (isset($error['file'])) { ?>
		<h1>程序抛出了一个异常</h1>
		<p style="margin-top:20px;text-align:center">程序猿刚刚被揍了，因为他写了这个BUG</p>
		<?php } else { ?>
			<p style="margin-top:20%;font-size:20px;text-align:center"><?php echo $result ?></p>
		<?php } ?>
	<?php } ?>
